...
local VERSION = "GLUE Version 19/11/22 (%s)";
local USAGE = ([=[
* command line syntax:
...
local PARSER = Cs{
  ...
  preloaded_name = P'"' * ((ANY - P'"')^1 / GPN) * P'"' +
                   P"'" * ((ANY - P"'")^1 / GPN) * P"'",
...
local function pml(ps, mt)
  ...
  local s = "";
  local t = {};
  for k in pairsByKey(mt) do insert(t, k) end;
  if #t > 0 then
    for i = 1, #t do
...
local function glue(text)
  ...
  return concat({stub, text, GLUESIG, linteger(#stub), linteger(#text)});
end;
...
local function printhelp()
  ...
  package.preload["Cmdl"] = nil; --luacheck: ignore
